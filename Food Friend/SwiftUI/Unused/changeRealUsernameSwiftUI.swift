//
//  changeRealUsernameSwiftUI.swift
//  Recall
//
//  Created by Tristan on 25/7/22.
//
...
struct changeRealUsernameSwiftUI: View {
    ...
    @State var usernameFieldIsEmpty: BannerModifier.BannerData = BannerModifier.BannerData(title: "Requirements Not Fulfilled", detail: "Please ensure that you entered your Current Username and a New Username", type: .Error)
    @State var currentUsernameIncorrect: BannerModifier.BannerData = BannerModifier.BannerData(title: "Current Username is Incorrect", detail: "Please ensure that the \"Current Username\" field has been filled out correctly.", type: .Error)
    @State var buttonWarnbanner: BannerModifier.BannerData = BannerModifier.BannerData(title: "Error Changing Username", detail: "That Username has already been taken, please try another one.", type: .Error)
    ...
    @State var usernameAlreadyYours: BannerModifier.BannerData = BannerModifier.BannerData(title: "Cant change Username to current one", detail: "You cannot change your Username to the Username you already have!", type: .Error)
    ...
    @AppStorage("currUsernames2", store: .standard) private var currUsernames2 = ""
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 15) {
                ...
                Spacer()
                Group {
                    TextField("Current Username", text: $username)
                        .padding(20)
                                       ...
                                       Firestore.firestore().collection("users").getDocuments() { (querySnapshot, err) in
                                           if let err = err {
                                               ...
                                               for document in querySnapshot!.documents {
                                                   if currUsernames2 == "" {
                                                       currUsernames2 = "\(document.get("username") ?? "")"
                                                   } else {
                                                       currUsernames2 = "\(currUsernames2), \(document.get("username") ?? "")"
                                                   }
                                                   print("amogus \(currUsernames2)")
                                               }
                                           }
                                       }
                                       
                                       let arrayOfUserz = currUsernames2.components(separatedBy: ", ")
                                       print("amogus2 \(arrayOfUserz)")
                                       
                                       if arrayOfUserz.contains(newUsername) {
...
struct changeRealUsernameSwiftUI_Previews: PreviewProvider {
    static var previews: some View {
        changeRealUsernameSwiftUI()
    }
}
