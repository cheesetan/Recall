//
//  imagemoveandselect.swift
//  Recall
//
//  Created by Tristan on 14/8/22.
//
/*
import SwiftUI
...
struct ImageMoveAndScaleSheet: View {
    ...
    private func HoleShapeMask() -> Path {
        ...
        let insetRect = CGRect(x: inset, y: UIScreen.main.bounds.width / 1.65, width: UIScreen.main.bounds.width - ( inset * 2 ), height: UIScreen.main.bounds.height / 2.4)
        var shape = Rectangle().path(in: rect)
        shape.addPath(RoundedRectangle(cornerSize: CGSize(width: 20, height: 20)).path(in: insetRect))
        return shape
    }
